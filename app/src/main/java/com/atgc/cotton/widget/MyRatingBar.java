...
public class MyRatingBar extends View {
    ...
    private void parseAttrs(AttributeSet attrs) {
        ...
        borderColor = arr.getColor(R.styleable.MyRatingBar_srb_borderColor, getResources().getColor(R.color.golden_stars));
        fillColor = arr.getColor(R.styleable.MyRatingBar_srb_fillColor, borderColor);
        starBackgroundColor = arr.getColor(R.styleable.MyRatingBar_srb_starBackgroundColor, Color.TRANSPARENT);
        backgroundColor = arr.getColor(R.styleable.MyRatingBar_srb_backgroundColor, Color.TRANSPARENT);

        pressedBorderColor = arr.getColor(R.styleable.MyRatingBar_srb_pressedBorderColor, borderColor);
        pressedFillColor = arr.getColor(R.styleable.MyRatingBar_srb_pressedFillColor, fillColor);
        pressedStarBackgroundColor = arr.getColor(R.styleable.MyRatingBar_srb_pressedStarBackgroundColor, starBackgroundColor);
        pressedBackgroundColor = arr.getColor(R.styleable.MyRatingBar_srb_pressedBackgroundColor, backgroundColor);

        numberOfStars = arr.getInteger(R.styleable.MyRatingBar_srb_numberOfStars, 5);

        starsSeparation = arr.getDimensionPixelSize(R.styleable.MyRatingBar_srb_starsSeparation, (int) valueToPixels(4, Dimension.DP));
        maxStarSize = arr.getDimensionPixelSize(R.styleable.MyRatingBar_srb_maxStarSize, Integer.MAX_VALUE);
        desiredStarSize = arr.getDimensionPixelSize(R.styleable.MyRatingBar_srb_starSize, Integer.MAX_VALUE);
        stepSize = arr.getFloat(R.styleable.MyRatingBar_srb_stepSize, Float.MAX_VALUE);
        starBorderWidth = arr.getFloat(R.styleable.MyRatingBar_srb_starBorderWidth, 5f);
        starCornerRadius = arr.getFloat(R.styleable.MyRatingBar_srb_starCornerRadius, 6f);

        rating = normalizeRating(arr.getFloat(R.styleable.MyRatingBar_srb_rating, 0f));
        isIndicator = arr.getBoolean(R.styleable.MyRatingBar_srb_isIndicator, false);
        drawBorderEnabled = arr.getBoolean(R.styleable.MyRatingBar_srb_drawBorderEnabled, true);
        gravity = Gravity.fromId(arr.getInt(R.styleable.MyRatingBar_srb_gravity, Gravity.Left.id));
    ...
    private void validateAttrs() {
        ...
        if (starBorderWidth <= 0) {
            throw new IllegalArgumentException(String.format("MyRatingBar initialized with invalid value for starBorderWidth. Found %f, but should be greater than 0",
                    starBorderWidth));
        }
        if (starCornerRadius < 0) {
            throw new IllegalArgumentException(String.format("MyRatingBar initialized with invalid value for starCornerRadius. Found %f, but should be greater or equal than 0",
                    starBorderWidth));
        }
    }
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
            Log.w("MyRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                if (listener != null) {
                    listener.onRatingChanged(MyRatingBar.this, rating, false);
                }
            }
    ...
    public interface OnRatingBarChangeListener {
         ...
         * lifting the touch.
         *
         * @param MyRatingBar The RatingBar whose rating has changed.
         * @param rating      The current rating. This will be in the range
         *                    0..numStars.
         * @param fromUser    True if the rating change was initiated by a user's
         *                    touch gesture or arrow key/horizontal trackbell movement.
         */
        void onRatingChanged(MyRatingBar MyRatingBar, float rating, boolean fromUser);
    ...
    public class AnimationBuilder {
        ...
        private AnimationBuilder(MyRatingBar ratingBar) {
