 ...
 *
 */
package com.boss66.meetbusiness.photoedit;
...
//    }
    public TextObject(Context context, String text, int x, int y,
                      Bitmap srBm, Bitmap rotateBm, Bitmap deleteBm) {
        super(text);
//        super(srcBm, x, y, rotateBm,
//                deleteBm);
        this.srcBm = srcBm;
        this.context = context;
        this.text = text;
        ...
        this.rotateBm = rotateBm;
        this.deleteBm = deleteBm;
        this.srcBm = Bitmap.createBitmap(srBm.getWidth(), srBm.getHeight(),
...
//                Bitmap.Config.ARGB_8888);
//        srcBm = Bitmap.createScaledBitmap(bitmap, textWidth, textSize * (lines.length) + 8, true);
//        Canvas canvas = new Canvas(srcBm);
//        canvas.drawARGB(0, 0, 0, 0);
        for (int i = 1; i <= lines.length; i++) {
//            canvas.drawText(lines[i - 1], 0, i * textSize, paint);
            canvas.drawText(lines[i - 1], 0, textSize, paint);
        }
        canvas.drawBitmap(srcBm, 0, 0, paint);
        setCenter();
